...
//   {category: 'soda', seasoning: 'diet coke', total: 5, qty: 1}
        ...
        try {
            ...
            this.cart = {};
            this.cart.pizzas = Array.from(pizzaSet.values());
            this.cart.wings = Array.from(wingsSet.values());
            this.cart.breadsticks = Array.from(breadsticksSet.values());
            this.cart.sodas = Array.from(sodaSet.values());
            this.cart.total = Currency.USD(totalPrice);
...
const Menu = class Menu {
    ...
    static getWingsTypesSet() {
        return new Set(['traditional', 'boness']);
    }
