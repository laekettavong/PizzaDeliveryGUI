...
const UserServices = class UserServices {
    ...
    * @param {callback}
    * 
    * @return specified user info
    */
    static getUserOLD(reqData, callback) {
        ...
        if(token) {
            UserContext.getContext(token, (err, context) => {
                if(!err && context) {
                    ...
                    callback(400, {Error : 'Could not create user context, invalid token'});
                }
            });
        } else {
    ...
    * @return
    */
    static deleteUserOLD(reqData, callback) {
        let email = StringHelper.isValidEmail(reqData.queryString.email);
        if(email){
            let token = StringHelper.isValidString(reqData.headers.token);
            TokenHelper.verifyToken(token, email, (isValidToken) => {
    ...
    * @return
    * 
    * @TODO cleanup (delete) any remaining artifacts associated with the user
    */
...
const UserContext = class UserContext {
    static getContext(token, callback) {
        //if(StringHelper.isValidString(token)){
                ...
                if(!err && tokenData && tokenData.expires > Date.now()){
                    FsHelper.read(Constants.USERS, tokenData.email, (err, userData) => {
                        if(!err && userData){
                            ...
                            console.log('Error : could not create user context');
                            callback(400, {Error : 'Could not find user'});
                        }
                    });
                } else {
                    console.log('Error : could not create user context, invalid token');
                    callback(500, {Error : 'Could not create user context, invalid token'});
                }
            });
        // } else {
