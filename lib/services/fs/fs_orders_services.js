        ...
        if(stripeToken){
            UserContext.getContext(reqData.headers.token, (err, context) => {
                if(!err && context) {
                    ...
                    callback(500, {Error : 'Could not create user context'});
                }
            });
        } else {
    ...
    static getOrder(reqData, callback) {
        UserContext.getContext(reqData.headers.token, (err, context) => {
            if(!err && context) {
                ...
                callback(400, {Error : 'Could not create user context'});
            }
        });
    }
    ...
    sendConfirmationEmail(callback) {
        ...
        mailgun.messages().send(emailObj, (err, body) => {
            if (!err) {
                ...
                console.log('Could not send order confirmation email', err);
